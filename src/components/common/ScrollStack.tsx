...
const ScrollStack = ({
  ...
  // Static offset references to prevent bounding client rect loops and vibration
  ...
  useLayoutEffect(() => {
    ...
    const recalculateOffsets = () => {
      ...
      cardOffsetsRef.current = activeCards.map((card) => {
      ...
      if (endElement) {
        const prevTransform = endElement.style.transform;
        ...
        endElement.style.transform = prevTransform;
      }
    ...
    // Capture phase load listener to catch lazy-loaded images finishing loading
    document.addEventListener("load", recalculateOffsets, true);
    // Listen to ScrollTrigger refresh updates
    ScrollTrigger.addEventListener("refresh", recalculateOffsets);
    ...
    return () => {
      ...
      document.removeEventListener("load", recalculateOffsets, true);
      ScrollTrigger.removeEventListener("refresh", recalculateOffsets);
      ...
      // Remove lingering Lenis CSS classes from <html> that cause overflow:hidden globally
